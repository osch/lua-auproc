# Activations script for add the auproc module to the lua path when
# building auproc using the Makefile.
# Source this into interactive shell by invoking ". activates.sh" from this directory
# This is not necessary if auproc is installed, e.g. via luarocks.

this_dir=$(pwd)

luaauproc_dir=$(cd "$this_dir"/..; pwd)

if [ ! -e "$luaauproc_dir/src/activate.sh" -o ! -e "$luaauproc_dir/src/main.c" ]; then

    echo '**** ERROR: ". activate.sh" must be invoked from "src" directory ***'

else

    echo "Setting lua paths for: $luaauproc_dir"

    add_lua_path="$luaauproc_dir/src/?.lua;$luaauproc_dir/src/?/init.lua"
    add_lua_cpath="$luaauproc_dir/src/build"
...
unset lua_cmd this_dir luaauproc_dir add_lua_path add_lua_cpath lua_version lua_path_vers vers default_version
